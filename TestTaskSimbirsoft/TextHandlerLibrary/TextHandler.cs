...
namespace HTMLHandlerLibrary
{
    ...
    /// </summary>
    public class HtmlHandler : HandlerLibrary
    {
        /// <summary>
        ...
        private static bool PrintIfWord(MatchCollection matchWordsInLine, int matchCounter, ref int charPos, StreamWriter outputStream, ICollection<string> dict)
        {
            for (var j = matchCounter; j < matchWordsInLine.Count; j++)
            {
                ...
                {
                    outputStream.Write(!dict.Contains(m.Value.ToLower()) ? m.Value : ("<strong><i>" + m.Value + "</strong></i>"));
                    charPos += m.Length - 1;
                    return true;
                }
            }
        ...
        /// <summary>
        /// Обеспечивает вывод обработанного текста в переданный файловый поток. Вызывается для каждого нового выходного файла (если их несколько).
        /// </summary>
        ...
        private static bool WorkWithNextFile(FileStream fileStream, IEnumerator<string> en, string resultFileName, ICollection<string> dict, ref string remainderLine)
        {
            ...
            {
                outputStream.Write("<html><head><title>Result-" + resultFileName +
                                       "</title><meta http-equiv=\"content-type\" content=\"text/html; charset=ansi\" /></head><body>");
                var curLinesCount = 0;
        ...
        public override void Run(string dictFileName, string textFileName, string resultFileName)
        {
            ...
            const string outFileExtension = ".html";
